...
const Testimonials = () => {
  ...
  return (
    <div className="testimonialWrapper flex flex-col items-center w-full">
            ...
            feedbacks.map((feedback) => (
              ...
              >
                <div className="userImg rounded-full border w-16 h-16">
                  <img src={feedback.profilePic} alt={feedback.username} className="w-full h-full object-cover rounded-full" />
                </div>
      ...
      <div onClick={handleYouCanBeNext}className="addFeedback relative right-[-40%]  bg-[#F77D03] text-white hover:bg-white hover:text-[#F77D03] hover:border-[#F77D03] font-bold rounded-[8px] px-[8px] py-[15px] border-2 border-transparent cursor-pointer transition-all duration-300 ease-in-out shadow-lg ">You Can Be Next</div>
    </div>
  );
};
